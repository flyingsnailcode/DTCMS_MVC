        ...
        public bool Add(Model.url_rewrite model)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.site))
                    xe.SetAttribute("site", model.site);
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == model.name.ToLower())
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.site))
                            xe.SetAttribute("site", model.site);
                        else if (xe.Attributes["site"] != null)
                            xe.Attributes["site"].RemoveAll();
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == attrValue.ToLower())
                    {
                        ...
                        if (xe.Attributes["site"] != null)
                            model.site = xe.Attributes["site"].Value;
