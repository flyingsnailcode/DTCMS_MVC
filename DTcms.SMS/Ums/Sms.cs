...
namespace DTcms.SMS.Ums
{
    ...
    public class Sms : ISms
    {
        ...
        /// <param name="password">企业编号</param>
        /// <param name="sendCount">同一手机单日最大允许发送量</param>
        /// <param name="ipCount">同一IP地址，单日最大允许提交请求次数</param>
        /// <param name="safeTotal">平台单日发送上限</param>
        public Sms(string uid, string password, string apicode, int sendCount, int ipCount, int safeTotal)
        {
            this.username = uid;
            if (!string.IsNullOrEmpty(password))
            {
                this.password = DESEncrypt.Decrypt(password);
            }
            else
            {
        ...
        public string GetAccountQuantity()
        {
            ...
            else
            {
                try
                {
                    ...
                    HttpItem item = new HttpItem()
                    {
                        URL = "http://sms.api.ums86.com:8899/sms/Api/SearchNumber.do" + string.Format("?SpCode={0}&LoginName={1}&Password={2}", this.apicode, this.username, this.password),
                        Encoding = Encoding.Default,
                        Method = "POST",
                        ContentType = "application/x-www-form-urlencoded"
                    };
                    ...
                    if (code == "0")
                    {
                        string total = QueryParameter(result.Html, "number");
                        txt.Append("<span>" + total + " 条</span>");
                    }
                    else
                    {
                        txt.Append("<span>错误信息：" + desc + "</span>");
                    }
                }
                catch
                {
            ...
            txt.Append(@"<span class=""Validform_checktip"">尚未申请？<a href=""http://www.ums86.com/"" target=""_blank"">请点击这里注册</a></span>");
            return txt.ToString();
        }
        ...
        /// 获取地址参数
        /// </summary>
        /// <param name="para">地址字符串</param>
        /// <param name="key">变量</param>
        /// <returns></returns>
        private string QueryParameter(string para, string key)
        {
            if (string.IsNullOrEmpty(para) || string.IsNullOrEmpty(key))
            {
                return string.Empty;
            }
            foreach (string s in para.Split('&'))
            {
                if (s.StartsWith(key + "="))
                {
