...
namespace Dapper
{
    public class Database : IDisposable
    {
        ...
        public IDbCommand CreateCommand(IDbConnection connection, IDbTransaction trans, string sql, params object[] args)
        {
        ...
        public IDbCommand CreateCommand(IDbConnection connection, string sql, params object[] args)
        {
            ...
            if (_providerName.Contains("MySql.Data.MySqlClient"))
            {
                sql = sql.Replace("top","limit");
            }
            else if (_providerName.Contains("System.Data.SqlClient"))
            {

            }
            else if (_providerName.Contains("Microsoft.Jet.OLEDB.4.0"))
            {
        ...
        public IDbCommand CreateCommand(IDbConnection connection, string sql, bool isExternal = false, params object[] args)
        {
            // Perform named argument replacements
            if (EnableNamedParams)
            {
                var new_args = new List<object>();
                //No execute sql check
                if (!isExternal)
                {
                    sql = ParametersHelper.ProcessParams(sql, args, new_args);
                }
        ...
        public int ExecuteSqlTran(IDbConnection connection, IDbTransaction trans, List<String> SQLStringList, params object[] args)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    ...
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        {
                            using (var cmd = CreateCommand(connection, trans, strsql, args))
                            {
        ...
        public int ExecuteSqlTran(List<String> SQLStringList, params object[] args)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    ...
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        if (strsql.Trim().Length > 1)
                        {
                            using (var cmd = CreateCommand(_sharedConnection, strsql, args))
                            {
                                int retv = cmd.ExecuteNonQuery();
                                OnExecutedCommand(cmd);
                                count += retv;
        ...
        /// <param name="args">对SQL中任何嵌入参数的参数</param>
        ...
        #region 事务处理
        public T ExecuteScalar<T>(IDbConnection connection, IDbTransaction trans, string sql, params object[] args)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(connection, trans, sql, args))
                    {
        ...
        public int Execute(IDbConnection connection, IDbTransaction trans, string sql, params object[] args)
        {
            try
            {
                OpenSharedConnection();
        ...
        public IEnumerable<T> Query<T>(IDbConnection connection, IDbTransaction trans, string sql, params object[] args)
        {
            if (EnableAutoSelect)
            ...
            try
            {
                using (var cmd = CreateCommand(connection, trans, sql, args))
                {
                    ...
                    var factory = pd.GetFactory(cmd.CommandText, connection.ConnectionString, 0, r.FieldCount, r) as Func<IDataReader, T>;
                    using (r)
                    {
                        while (true)
                        {
        ...
        public DataSet QueryFillDataSets(IDbConnection connection, IDbTransaction trans, string sql, CommandType cmdType, params SqlParameter[] pms)
        {
            try
            {
                OpenSharedConnection();
                DataSet ds = new DataSet();
                using (var cmd = CreateCommand2(connection, trans, sql, true, pms))
                {
                    ...
                    using (DbDataAdapter dbDataAdapter = da)
                    {
        ...
        public System.Data.DataSet QueryFillDataSet(IDbConnection connection, IDbTransaction trans, string sql)
        {
            return QueryFillDataSets(connection, trans, sql, CommandType.Text);
        }

        public System.Data.DataSet QueryFillDataSet(Sql sql)
        {
            return QueryFillDataSet(sql.SQL, CommandType.Text);
        }

        public IEnumerable<T> Query<T>(IDbConnection connection, IDbTransaction trans, Sql sql)
        {
            return Query<T>(connection, trans, sql.SQL, sql.Arguments);
        }
        ...
        /// <param name="args">对SQL语句中任何嵌入参数的参数</param>
        /// <returns>结果集中的第一条记录</returns>
        public T QueryFirst<T>(IDbConnection connection, IDbTransaction trans, string sql, params object[] args)
        {
            return Query<T>(connection, trans, sql, args).First();
        }
        #endregion
        ...
        /// 执行查询并返回结果集中第一行的第一列
        /// </summary>
        /// <typeparam name="T">结果值应该被转换的类型</typeparam>
        /// <param name="sql">执行SQL查询</param>
        /// <param name="args">对SQL中任何嵌入参数的参数</param>
        /// <returns>标量值转换为T</returns>
        ...
        /// </summary>
        /// <typeparam name="T">The Type representing a row in the result set</typeparam>
        /// <param name="sql">SQL查询 to execute</param>
        /// <param name="args">对SQL中任何嵌入参数的参数</param>
        ...
        /// <returns>A Page of results</returns>
        /// <remarks>
        /// Dapper will automatically modify the supplied SELECT statement to only retrieve the
        ...
        public DataSet QueryFillDataSet(string sql, CommandType cmdType, params SqlParameter[] pms)
        {
            try
            {
                OpenSharedConnection();
                DataSet ds = new DataSet();
                using (var cmd = CreateCommand1(_sharedConnection, sql, true, pms))
                {
                    cmd.CommandType = cmdType;
                    if (pms != null && pms.Count() > 0)
                    {
        ...
        /// <returns></returns>
        public System.Data.DataSet QueryFillDataSet(string sql, params SqlParameter[] pms)
        {
            return QueryFillDataSet(sql, CommandType.Text, pms);
        }

        public IDbCommand CreateCommand1(IDbConnection connection, string sql, bool isExternal = false, params object[] args)
        {
            // Perform named argument replacements
            if (EnableNamedParams)
            {
        ...
        public IDbCommand CreateCommand2(IDbConnection connection, IDbTransaction trans, string sql, bool isExternal = false, params object[] args)
        {
            // Perform named argument replacements
            if (EnableNamedParams)
            {
        ...
        /// <summary>
        /// 返回具有指定主键值的记录，如果未找到，则返回默认值
        /// </summary>
        /// <typeparam name="T">The Type representing a row in the result set</typeparam>
        /// <param name="primaryKey">The primary key value of the record to fetch</param>
        /// <returns>The single record matching the specified primary key value</returns>
        /// <remarks>
        /// 如果没有指定主键值的记录，则返回默认（T）（通常为空）
        /// </remarks>
        ...
        /// <param name="sql">SQL查询</param>
        /// <param name="args">对SQL语句中任何嵌入参数的参数</param>
        /// <returns>如果没有匹配行，则匹配指定的主键值或默认值（t）的单个记录</returns>
        ...
        /// <summary>
        /// 运行查询并返回第一个记录，如果没有匹配记录，则返回默认值
        /// </summary>
        /// <typeparam name="T">表示结果集中的一行的类型</typeparam>
        /// <param name="sql">SQL查询</param>
        /// <param name="args">对SQL语句中任何嵌入参数的参数</param>
        /// <returns>结果集中的第一条记录，如果没有匹配的行，则默认为（T）</returns>
        ...
        /// <param name="primaryKeyName">主键列的名称 of the table</param>
        ...
        /// </summary>
        /// <param name="tableName">要删除的表的名称</param>
        /// <param name="primaryKeyName">主键列的名称</param>
        /// <param name="poco">POCO对象的主键值将用于删除行</param>
        /// <returns>受影响的行数</returns>
        ...
        /// <param name="tableName">要删除的表的名称</param>
        /// <param name="primaryKeyName">主键列的名称</param>
        /// <param name="poco">POCO对象的主键值将用于删除行 (或NULL使用所提供的主键值)</param>
        /// <param name="primaryKeyValue">主键的鉴定记录被删除的值（或者为空，或者把这个值从POCO实例）</param>
        /// <returns>受影响的行数</returns>
        ...
        /// </summary>
        /// <param name="poco">指定要删除的表名和行的主键值的POCO对象</param>
        /// <returns>受影响的行数</returns>
        ...
        /// Performs an SQL Delete
        /// </summary>
        /// <typeparam name="T">属性标识要在删除中使用的表和主键的POCO类</typeparam>
        /// <param name="pocoOrPrimaryKey">要删除的行的主键的值</param>
        /// <returns></returns>
        ...
        /// 执行SQL删除
        /// </summary>
        /// <typeparam name="T">属性的POCO类指定了要从中删除的表的名称</typeparam>
        /// <param name="sql">标识要删除的行的SQL条件子句（即：“DELETE FROM tablename”之后的所有内容</param>
        /// <param name="args">对SQL中任何嵌入参数的参数</param>
        ...
        /// <param name="sql">SQL查询 to be executed</param>
        /// <param name="args">对SQL中任何嵌入参数的参数</param>
        ...
        /// <summary>
        /// When set to true, Dapper will automatically create the "SELECT columns" part of any query that looks like it needs it
        /// </summary>
        ...
        public IDbConnection _sharedConnection;
    ...
    Usage:   
            ...
            new Dapper.Database("<connstring>", new Dapper.OracleProvider())
    ...
    assembly containing OracleProvider.cs)
        ...
        <system.data>
            <DbProviderFactories>
                <add name="Dapper Oracle Provider" invariant="Oracle" description="Dapper Oracle Provider" 
                                type="Dapper.OracleProvider, ASSEMBLYNAME" />
            </DbProviderFactories>
        </system.data>
    ...
    */
    ...
    /// <summary>
    /// 把字符串在这个类的一个实例强制使用 DBType.AnsiString
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// 通常，ColumnInfo会自动从POCO对象及其属性的属性中填充。 它可以
    /// 但是也可以从IMapper界面返回，以便在DB和POCO之间提供您自己的绑定。
    /// </remarks>
    ...
    /// <summary>
    /// IMapper provides a way to hook into Dapper's Database to POCO mapping mechanism to either
    ...
    /// Dapper through the static method Mappers.Register()
    /// </remarks>
    ...
    /// <summary>
    /// This static manages registation of IMapper instances with Dapper
    /// </summary>
    ...
    public class Sql
    {
        ...
        public Sql Top(params object[] tables)
        {
            return Append(new Sql("TOP " + String.Join(", ", (from x in tables select x.ToString()).ToArray())));
        }
    ...
    /// <summary>
    /// StandardMapper is the default implementation of IMapper used by Dapper
    /// </summary>
    ...
    namespace Internal
    {
        ...
        abstract class DatabaseType
        {
            ...
            /// <param name="args">对SQL中任何嵌入参数的参数 query</param>
            ...
            /// <summary>
            /// 查看正在使用的类型和提供程序名称，并实例化一个合适的数据库类型实例
            /// </summary>
