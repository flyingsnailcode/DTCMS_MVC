...
namespace DTcms.Web.Mvc.UI.Controllers
{
    public partial class BaseController : Controller
    {
        ...
        {
            return int.Parse(new BLL.orders().GetCount("count(*) as H", strwhere));
        }
        ...
        public List<Model.orders> get_order_list(int top, string strwhere)
        {
            return new BLL.orders().GetModelList(top, strwhere,"", "add_time desc,id desc");
        }
        ...
        public List<Model.orders> get_order_list(int page_size, int page_index, string strwhere, out int totalcount)
        {
            return new BLL.orders().GetList(page_size, page_index, strwhere, "add_time desc,id desc", out totalcount);
        }
        ...
        {
            string _title = "";
            Model.orders model = new BLL.orders().Get(_id);
            switch (model.status)
            {
        ...
        {
            Model.orders model = new BLL.orders().Get(order_id);
            if (model == null)
            {
                return false;
            }
            ...
            Model.payment payModel = new BLL.payment().Get(model.payment_id);
            if (payModel == null)
            {
                return false;
            }
