...
namespace DTcms.Web.Mvc.UI
{
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
           ...
           //}
            ...
            if (siteConfig.staticstatus == 0)
            {
                #region 站点不开启重写处理方法===========================
                //遍历URL字典，匹配URL页面部分
                foreach (Model.url_rewrite model in SiteUrls.GetUrls(sitePath).Urls)
                {
                    //查找到与页面部分匹配的节点
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        context.RewritePath(string.Format("{0}{1}/{2}{3}", siteConfig.webpath, DTKeys_Extension.DIRECTORY_REWRITE_MVC, sitePath, requestPage));
                        return;
                    }
                }
                #endregion
            ...
            else
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetUrls(sitePath).Urls)
                {
                    ...
                    {
                        //映射到站点目录
                        result = string.Format("{0}/{1}", siteConfig.webpath, model.page.Replace(".cshtml", "/index"));
                    ...
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                        ...
                        bool a = Regex.IsMatch(requestPage, string.Format("^/{0}$", newPattern), RegexOptions.None | RegexOptions.IgnoreCase);
                        bool b = model.page == "index.cshtml" ? true : false;
                        bool c = Regex.IsMatch(requestPage, string.Format("^/{0}$", item.pattern), RegexOptions.None | RegexOptions.IgnoreCase);
                        //如果与URL节点匹配则重写
                        if (Regex.IsMatch(requestPage, string.Format("^/{0}$", newPattern), RegexOptions.None | RegexOptions.IgnoreCase) || (model.page == "index.cshtml" && Regex.IsMatch(requestPage, string.Format("^/{0}$", item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            //如果开启生成静态、不是移动站点且是频道页或首页,则映射重写到HTML目录
                            if (siteConfig.staticstatus == 2 && (model.channel.Length > 0 || model.type.ToLower() == "index")) //频道页
                            {
    ...
    public class SiteUrls
    {
        ...
        //构造函数
        private SiteUrls(string sitePath)
        {
            Urls = new ArrayList();
            ...
            {
                if (string.IsNullOrEmpty(model.site) || model.site == sitePath)
                {
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
        ...
        //返回URL字典
        public static SiteUrls GetUrls(string sitePath)
        {
            ...
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(DTKeys.CACHE_SITE_HTTP_MODULE, new SiteUrls(sitePath), Utils.GetXmlMapPath(DTKeys.FILE_URL_XML_CONFING));
    ...
    public class SiteDomains
    {
        ...
        public SiteDomains()
        {
            SiteList = new BLL.sites().GetModelList(0, string.Empty, string.Empty, "sort_id asc,id desc");
