...
namespace DTcms.BLL
{
    public class url_rewrite
    {
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public bool Exists(string site, string name)
        {
            if (string.IsNullOrEmpty(name))
            {
                return false;
            ...
            foreach (Model.url_rewrite modelt in GetListAll())
            {
                if (modelt.site == site && modelt.name == name)
                {
                    return true;
                }
            }
        ...
        {
            return GetListAll().Find(p => p.channel == channel && p.type == attrType);
        }

        /// <summary>
        /// 根据网站名称、频道名称和类别返回一条URL映射
        /// </summary>
        public Model.url_rewrite GetInfo(string site, string channel, string attrType)
        {
            return GetListAll().Find(p => p.site == site && p.channel == channel && p.type == attrType);
        }
