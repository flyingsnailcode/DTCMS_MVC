...
namespace DTcms.BLL
{
    public partial class siteconfig
    {
        ...
        {
            //CacheHelper.Remove(DTKeys.CACHE_SITE_CONFIG);
            Model.siteconfig model = CacheHelper.Get<Model.siteconfig>(DTKeys.CACHE_SITE_CONFIG);
            if (model == null)
            {
                CacheHelper.Insert(DTKeys.CACHE_SITE_CONFIG, dal.loadConfig(Utils.GetXmlMapPath(DTKeys.FILE_SITE_XML_CONFING)),Utils.GetXmlMapPath(DTKeys.FILE_SITE_XML_CONFING));
